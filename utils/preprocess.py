...
def extract_landmarks(image_path):
    ...
    if image is None:
        return None
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) #from bgr to rgb
    results = hands.process(image_rgb)

    if results.multi_hand_landmarks:
        landmarks = results.multi_hand_landmarks[0] #only gets first handsladmark 
        return np.array([[lm.x, lm.y, lm.z] for lm in landmarks.landmark]).flatten() #puts as XYZ
    return None
...
def process_dataset(dataset_path):
    ...
    for label in tqdm(os.listdir(dataset_path), desc="Processing labels"):
        label_path = os.path.join(dataset_path, label)  #path to the label folder
        if not os.path.isdir(label_path):
            continue

        for image_name in tqdm(os.listdir(label_path), desc=f"Processing images for label {label}", leave=False):
            image_path = os.path.join(label_path, image_name) #path to the individual image
...
def prepare_specific_datasets():
    ...
    if not all_data or not all_labels: #i used this for gestures
    ...
    # np.save(os.path.join(base_path, "..", "SVM-SignLanguage", "data_features.npy"), all_data)
    # np.save(os.path.join(base_path, "..", "SVM-SignLanguage", "data_labels.npy"), all_data)
